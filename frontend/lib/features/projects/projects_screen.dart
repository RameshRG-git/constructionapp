...
class _ProjectsScreenState extends State<ProjectsScreen> {
  ...
  Future<void> _loadProjects() async {
    ...
    try {
      final response = await ApiRegistry.projects.listProjects(
  ...
  Future<void> _showProjectDialog({Map<String, dynamic>? existing}) async {
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                if (existing == null) {
                  await ApiRegistry.projects.createProject(payload);
                } else {
                  await ApiRegistry.projects.updateProject(existing['id'] as int, payload);
                }
                if (!mounted) {
                  return;
                }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(child: Text('Projects', style: theme.textTheme.headlineMedium)),
            FilledButton.icon(
        ...
        Text('List, filter, sort, add, and modify projects from PostgreSQL.', style: theme.textTheme.bodyLarge),
        ...
        Wrap(
          ...
          children: [
            ...
            FilledButton.icon(onPressed: _loadProjects, icon: const Icon(Icons.filter_alt), label: const Text('Apply')),
          ],
        ),
        const SizedBox(height: 16),
        Expanded(
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                subtitle: Text(
                                  '${project['site_location'] ?? '-'} • Owner: ${project['owner_name'] ?? '-'}',
                                ),
                                trailing: Wrap(
                                  spacing: 8,
                                  crossAxisAlignment: WrapCrossAlignment.center,
                                  children: [
                                    Chip(label: Text(project['status']?.toString() ?? '-')),
                                    IconButton(
                                      icon: const Icon(Icons.edit_outlined),
                                      onPressed: () => _showProjectDialog(existing: project),
                                    ),
                                  ],
                                ),
                              );
