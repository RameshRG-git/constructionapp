...
class AdvanceService:
    ...
    @staticmethod
    def due_outstanding_advances(tenant_name, employee_name, as_of_date):
        ...
        advances = EmployeeAdvance.query.filter(
            ...
            EmployeeAdvance.employee_name == employee_name,
    ...
    @staticmethod
    def outstanding_balance(tenant_name, employee_name):
        advances = EmployeeAdvance.query.filter(
            EmployeeAdvance.tenant_name == tenant_name,
            EmployeeAdvance.employee_name == employee_name,
        ).all()
    ...
    @staticmethod
    def preview_recovery(tenant_name, employee_name, week_end_date, net_earned):
        """Non-committing projection of how much would be recovered this week."""
        remaining = float(net_earned or 0)
        total = 0.0
        for advance in AdvanceService.due_outstanding_advances(tenant_name, employee_name, week_end_date):
            if remaining <= 0:
                break
    ...
    @staticmethod
    def apply_recovery(tenant_name, employee_name, week_start_date, week_end_date, net_earned):
    ...
    @staticmethod
    def recovery_already_recorded(tenant_name, employee_name, week_start_date):
        existing = AdvanceRecovery.query.filter(
            AdvanceRecovery.tenant_name == tenant_name,
            AdvanceRecovery.employee_name == employee_name,
