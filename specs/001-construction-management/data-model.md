...
- Fields: id, project_id, item_name, category, unit_of_measure, current_quantity, minimum_quantity,
  low_stock_flag, storage_location, created_at, updated_at.
- Relationships: belongs to a project; has many inventory transactions.
...
- Fields: id, project_id, assignee_type, assignee_name, title, description, priority, status, due_date,
  estimated_hours, created_at, updated_at.
- Status values: open, in_progress, blocked, completed.
- Relationships: belongs to a project.

### Budget Record
- Represents planned and actual financial tracking for a project.
- Fields: id, project_id, category_name, planned_amount, actual_amount, remaining_amount,
...
- Fields: project status, inventory risk count, workload distribution, budget variance, last updated.
- Relationships: derived from project, inventory item, work assignment, and budget record data.
- Validation: computed values must stay in sync with underlying records.

## Relationships
- One project has many inventory items.
- One project has many work assignments.
- One project has many budget records.
- One inventory item has many inventory transactions.
- One user role can be assigned to many users.

## State Transitions
- Project: planned -> active -> on_hold -> active -> closed.
...
## Validation Rules
- Project names must be present and unique within the operating workspace.
...
- Closed projects should reject new operational changes except authorized reopening workflows.
